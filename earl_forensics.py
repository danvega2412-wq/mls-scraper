...
def run_earl_audit(mls):
    ...
    parts.insert(0, {"text": "Scan these MLS listing photos and return a JSON object with these exact keys. Only flag TRUE if you are confident. Be strict. bad_lead_photo: true if first photo is NOT a proper front exterior. poor_photo_sequence: true ONLY if the sequence is extremely and obviously chaotic - like repeatedly jumping between exterior and interior shots with no pattern at all. Do NOT flag if photos simply start outside and move inside room by room. When in doubt, do NOT flag this. no_professional_photography: true if photos look like phone snapshots. poor_photography_quality: true if professional camera but poor execution. floor_plan_in_photos: true ONLY if a photo shows an actual interior floor plan blueprint of the house itself showing room layouts and walls. Do NOT flag neighborhood maps, site plans, subdivision maps, or aerial views. virtual_tour_in_photos: true if ANY photo shows a Matterport or 3D tour screenshot. visual_notes: brief summary. Return ONLY valid JSON, nothing else."})
