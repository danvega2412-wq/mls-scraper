...
def run_earl_audit(mls):
    ...
    parts.insert(0, {"text": "Scan these MLS listing photos and return a JSON object with these exact keys. Only flag TRUE if you are confident. Be strict. bad_lead_photo: true ONLY if first photo is clearly an interior room, a blurry/dark shot, a vertical phone photo with dead space on sides, or has no subject. A clean front exterior shot - even if not perfect - should NEVER be flagged. When in doubt, do NOT flag. poor_photo_sequence: true ONLY if the sequence is extremely and obviously chaotic - like repeatedly jumping between exterior and interior shots with no pattern at all. Do NOT flag if photos simply start outside and move inside room by room. When in doubt, do NOT flag this. no_professional_photography: true ONLY if photos are clearly casual phone snapshots with no editing - poor lighting, tilted angles, clutter visible. Do NOT flag if photos are decent quality even if not magazine-level. When in doubt, do NOT flag. poor_photography_quality: true ONLY if photos show obvious problems like severe overexposure, extreme darkness, heavy motion blur, or badly distorted compositions. Wide-angle lens distortion is normal in real estate photography - do NOT flag it. When in doubt, do NOT flag. floor_plan_in_photos: true ONLY if a photo shows an actual interior floor plan blueprint of the house itself showing room layouts and walls. Do NOT flag neighborhood maps, site plans, subdivision maps, or aerial views. virtual_tour_in_photos: true if ANY photo shows a Matterport or 3D tour screenshot. visual_notes: brief summary. Return ONLY valid JSON, nothing else."})
